...
// Note that we're reusing the welcome illustation here, any changes to it
// will have be reflected in the welcome flow as well.
const BottomImageUri = `file:///${__dirname}/static/welcome-illustation-left-bottom.svg`

const issuesUri = 'https://github.com/sbg-projectx/gitburn/issues'
...
export class CrashApp extends React.Component<ICrashAppProps, ICrashAppState> {
  ...
  private renderTitle() {
    const message = this.state.type === 'launch'
      ? 'GitBurn failed to launch'
      : 'GitBurn encountered an error'
  ...
  private renderDescription() {
    if (this.state.type === 'launch') {
      return (
        <p>
          GitBurn encountered a catastrophic error that prevents it from
          launching. This has been reported to the team, but if you encounter this
          repeatedly please report this to the
          GitBurn <LinkButton uri={issuesUri}>issue tracker</LinkButton>
        </p>
      )
    } else {
      return (
        <p>
          GitBurn has encountered an unrecoverable error and will need to restart.
          This has been reported to the team, but if you encounter this repeatedly please
          report this issue to the GitBurn <LinkButton uri={issuesUri}>issue tracker</LinkButton>
        </p>
      )
    }
  }
