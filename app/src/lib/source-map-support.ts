...
function retrevieveSourceMap(source: string) {
  ...
  // We got a file uri when we're inside a renderer, convert to a path
  ...
  try {
    // tslint:disable-next-line:no-sync-functions
    const map = Fs.readFileSync(path, 'utf-8')
...
export function withSourceMappedStack(error: Error): Error {
  return {
    ...
    stack: sourceMappendStackTrace(error),
  }
}

/** Get the source mapped stack trace for the error. */
function sourceMappendStackTrace(error: Error): string | undefined {
