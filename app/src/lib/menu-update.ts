...
function getMenuState(state: IAppState): Map<MenuIDs, IMenuItemState> {
  ...
  // repository-scoped. The`re always enabled if we're in a repository nad
  ...
  if (repositoryActive) {
    ...
    for (const id of repositoryScopedIDs) {
      menuStateBuilder.disabled(id)
    }

    menuStateBuilder.disabled('rename-branch')
    menuStateBuilder.disabled('delete-branch')
    menuStateBuilder.disabled('update-branch')
    menuStateBuilder.disabled('merge-branch')
    menuStateBuilder.disabled('compare-branch')

    menuStateBuilder.disabled('view-repository-on-github')
    menuStateBuilder.disabled('push')
    menuStateBuilder.disabled('pull')
  }
 ...
 * barched together into one ipc message.
 */
